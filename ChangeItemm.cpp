...
bool ChangeItem::updRelease(const char* id, const char* newRelease) {
    ...
    while (getline(dbFileIn, line)) {
        if (line.find(id) != string::npos) {
            recordFound = true;
            string newReleaseStr(newRelease);
            newReleaseStr.resize(sizeof(releaseID.id) - 1, ' '); // Pad with spaces if necessary
            line.replace(110, sizeof(releaseID.id) - 1, newReleaseStr); // Assuming release ID starts at position 110
        }
        lines.push_back(line);
    }
...
bool ChangeItem::updPriority(const char* id, int newPriority) {
    ...
    while (getline(dbFileIn, line)) {
        if (line.find(id) != string::npos) {
            recordFound = true;
            string newPriorityStr = to_string(newPriority);
            newPriorityStr.resize(1, ' '); // Ensure it fits within the space
            line.replace(125, 1, newPriorityStr); // Assuming priority is a single character at position 125
        }
        lines.push_back(line);
    }
...
bool ChangeItem::updDesc(const char* id, const char* newDescription) {
    ...
    while (getline(dbFileIn, line)) {
        if (line.find(id) != string::npos) {
            recordFound = true;
            string newDescStr(newDescription);
            newDescStr.resize(30, ' '); // Assuming description is max 30 characters
            line.replace(60, 30, newDescStr); // Assuming description starts at position 60
        }
        lines.push_back(line);
    }
